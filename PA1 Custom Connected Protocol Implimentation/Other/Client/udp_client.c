...
int main(int argc, char **argv) {
    ...
    int check; // valid sie check for connected protocol
        ...
        else
        {
            ...
            if (strcmp(buf,"get") == 0)
            {
                ...
                while(1)
                {
                    ...
                    /* if the client receives an invalid size from the server the packet has been corrupted
                    * a value of -1 is sent to the server indicating that a resend is required */
                    if (size > 1023)
                    {
                        check = -1;
                        n = sendto(sockfd, &check, sizeof(int), 0, &serveraddr, serverlen);
                    }

                    /* if a valid size is received than a 0 value is send indication transmission can begin */
                    else
                    {
                        check = 0;
                        n = sendto(sockfd, &check, sizeof(int), 0, &serveraddr, serverlen);
                    ...
                    /* loop to continue checking for valid packets 
                    * transmission will begin once a valid size is received */
                    while (size > 1023)    
                    {
            ...
            else if (strcmp(buf,"put") == 0)
            {
                ...
                if (fp != NULL) // check to make sure filename exists
                {
                    while(!feof(fp)) // while not at the end of the file
                    {
                        ...
                        /* 
                        * implimenting connected protocol
                        * checks to make sure the size of the packet received by the server is valid
                        * if invalid size is resent. if valid transmission begins
                        */
                        n = recvfrom(sockfd, &check, sizeof(int), 0, &serveraddr, &serverlen); // check to ensure server recieved correct file size (TCP)
                        while (check == -1)
                        {
                            n = sendto(sockfd, &size, BUFSIZE, 0, &serveraddr, serverlen);
                            n = recvfrom(sockfd, &check, sizeof(int), 0, &serveraddr, &serverlen);
                        }
